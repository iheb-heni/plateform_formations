...
public class DashboardEtudiantServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (session != null) {
            ...
            // Vérifie que l'utilisateur est authentifié et qu'il a le rôle "etudiant"
            if (utilisateur != null && "etudiant".equalsIgnoreCase(utilisateur.getRole())) {
                // Prépare les données nécessaires pour le tableau de bord (si nécessaire)

                // Redirige vers la page du tableau de bord
                RequestDispatcher dispatcher = req.getRequestDispatcher("./dashboardEtudiant.jsp");
